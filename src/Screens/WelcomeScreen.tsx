...
const WelcomeScreen = () => {
    ...
    const {textRegular, textMedium, textBeige200, textBeige100, textCenter} = Fonts

    const {t} = useTranslation()

    return (
        <BackgroundImageContainer source={Images.onBoarding}>
            <LinearGradient style={[Layout.fill, Layout.colCenter, {justifyContent: "flex-end"}]}
                            colors={[Colors.primary + '80', Colors.primary + '80', Colors.primary + 'F2']}>
                <Brand width={Dim.getHorizontalDimension(270)} height={Dim.getDimension(300)}/>
                <Text
                    style={[textRegular, textMedium, textBeige100, textCenter,styles.marginT80, {width: Dim.getHorizontalDimension(320)}]}>
                ...
                <View style={[Layout.row, Layout.justifyContentBetween,styles.bottomFooter]}>
